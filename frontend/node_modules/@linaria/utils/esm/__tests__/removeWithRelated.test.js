...
const run = rawCode => {
  ...
  const visitor = path => {
    if (!path.node.leadingComments?.length || path.node.leadingComments[0].value.trim() !== 'remove') {
      return;
    }
    ...
    if (path.listKey && typeof path.key === 'number' && path.key > 0) {
      ...
      if (prevNode.node.trailingComments?.includes(comment)) {
        ...
        prevNode.node.trailingComments = prevNode.node.trailingComments?.filter(c => c !== comment);
      }
    }
