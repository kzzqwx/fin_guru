...
function importFromVariableDeclarator(path, isSync) {
  ...
  // What else it can be?
  debug('evaluator:collectExportsAndImports:importFromVariableDeclarator', 'Unknown type of id', id.node.type);
  return [];
}
const findIIFE = path => {
...
function exportFromVariableDeclarator(path) {
  ...
  // What else it can be?
  debug('evaluator:collectExportsAndImports:exportFromVariableDeclarator', 'Unknown type of id', id.node.type);
  return {};
}
...
function collectFromRequire(path, state) {
  ...
  if (container.isCallExpression() && key === 0) {
    ...
    if (!imported) {
      ...
      debug('evaluator:collectExportsAndImports', 'Unknown wrapper of require', container.node.callee);
      return;
    }
    ...
    if (!variableDeclarator.isVariableDeclarator()) {
      ...
      debug('evaluator:collectExportsAndImports', 'Unexpected require inside', variableDeclarator.node.type);
      return;
    }
    const id = variableDeclarator.get('id');
    if (!id.isIdentifier()) {
      debug('evaluator:collectExportsAndImports', 'Id should be Identifier', variableDeclarator.node.type);
      return;
    }
    if (imported === 'import:*') {
  ...
  if (container.isMemberExpression()) {
    ...
    if (!property.isIdentifier() && !property.isStringLiteral()) {
      debug('evaluator:collectExportsAndImports', 'Property should be Identifier or StringLiteral', property.node.type);
      return;
    }
    const {
      parentPath: variableDeclarator
...
function unfoldNamespaceImport(importItem) {
  ...
  for (const referencePath of binding?.referencePaths ?? []) {
    ...
    if (parentPath?.isCallExpression() && referencePath.listKey === 'arguments') {
      ...
      debug('evaluator:collectExportsAndImports:unfoldNamespaceImports', 'Unknown import type', importType);
      result.push(importItem);
      continue;
    }
    ...
    debug('evaluator:collectExportsAndImports:unfoldNamespaceImports', 'Unknown reference', referencePath.node.type);
    result.push(importItem);
    break;
  }
  return result;
...
function collectFromExportSpecifier(path, source, state) {
  ...
  debug('evaluator:collectExportsAndImports:collectFromExportSpecifier', 'Unprocessed ExportSpecifier', path.node.type);
}
