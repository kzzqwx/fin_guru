...
function importFromVariableDeclarator(path, isSync) {
  ...
  if (!isSync) {
    ...
    (0, _logger.debug)('evaluator:collectExportsAndImports', '`import` should be awaited');
    return [];
  }
  ...
  // What else it can be?
  (0, _logger.debug)('evaluator:collectExportsAndImports:importFromVariableDeclarator', 'Unknown type of id', id.node.type);
  return [];
}
const findIIFE = path => {
...
function exportFromVariableDeclarator(path) {
  ...
  if (!init.node && id.isIdentifier()) {
    ...
    const iife = [...binding.referencePaths, ...binding.constantViolations, binding.path].map(findIIFE).find(_isNotNull.default);
    if (!iife) {
      return {};
    }
    return {
  ...
  // What else it can be?
  (0, _logger.debug)('evaluator:collectExportsAndImports:exportFromVariableDeclarator', 'Unknown type of id', id.node.type);
  return {};
}
...
function collectFromRequire(path, state) {
  ...
  if (container.isCallExpression() && key === 0) {
    ...
    if (!imported) {
      ...
      (0, _logger.debug)('evaluator:collectExportsAndImports', 'Unknown wrapper of require', container.node.callee);
      return;
    }
    ...
    if (!variableDeclarator.isVariableDeclarator()) {
      ...
      (0, _logger.debug)('evaluator:collectExportsAndImports', 'Unexpected require inside', variableDeclarator.node.type);
      return;
    }
    const id = variableDeclarator.get('id');
    if (!id.isIdentifier()) {
      (0, _logger.debug)('evaluator:collectExportsAndImports', 'Id should be Identifier', variableDeclarator.node.type);
      return;
    }
    if (imported === 'import:*') {
  ...
  if (container.isMemberExpression()) {
    ...
    if (!property.isIdentifier() && !property.isStringLiteral()) {
      (0, _logger.debug)('evaluator:collectExportsAndImports', 'Property should be Identifier or StringLiteral', property.node.type);
      return;
    }
    const {
      parentPath: variableDeclarator
...
function collectFromVariableDeclarator(path, state) {
  ...
  path.traverse({
    Identifier(identifierPath) {
      if ((0, _isRequire.default)(identifierPath)) {
...
function getGetterValueFromDescriptor(descriptor) {
  const props = descriptor.get('properties').filter((0, _isTypedNode.default)('ObjectProperty'));
...
function unfoldNamespaceImport(importItem) {
  ...
  for (const referencePath of (_binding$referencePat = binding === null || binding === void 0 ? void 0 : binding.referencePaths) !== null && _binding$referencePat !== void 0 ? _binding$referencePat : []) {
    ...
    if (parentPath !== null && parentPath !== void 0 && parentPath.isCallExpression() && referencePath.listKey === 'arguments') {
      ...
      (0, _logger.debug)('evaluator:collectExportsAndImports:unfoldNamespaceImports', 'Unknown import type', importType);
      result.push(importItem);
      continue;
    }
    ...
    (0, _logger.debug)('evaluator:collectExportsAndImports:unfoldNamespaceImports', 'Unknown reference', referencePath.node.type);
    result.push(importItem);
    break;
  }
  return result;
...
function collectFromExportSpecifier(path, source, state) {
  ...
  (0, _logger.debug)('evaluator:collectExportsAndImports:collectFromExportSpecifier', 'Unprocessed ExportSpecifier', path.node.type);
}
...
function collectFromAssignmentExpression(path, state) {
  ...
  if (!exported) return;
  if (!right.isCallExpression() || !(0, _isRequire.default)(right.get('callee'))) {
    // eslint-disable-next-line no-param-reassign
