export declare const classes: {
    comboboxTargeText: string;
    comboboxTargetLabel: string;
    comboboxTargetInput: string;
    comboboxTargetArrow: string;
    arrowInverse: string;
    comboboxItemIsSelected: string;
    comboboxItemIsDisabled: string;
    hasChips: string;
    innerLabelUp: string;
};
export declare const tokens: {
    background: string;
    boxShadow: string;
    width: string;
    height: string;
    borderRadius: string;
    paddingTop: string;
    paddingRight: string;
    paddingBottom: string;
    paddingLeft: string;
    itemBackground: string;
    itemColor: string;
    itemBackgroundHover: string;
    itemBackgroundSelected: string;
    itemColorSelected: string;
    itemBackgroundSelectedHover: string;
    itemColorSelectedHover: string;
    itemWidth: string;
    itemHeight: string;
    itemBorderRadius: string;
    itemPaddingTop: string;
    itemPaddingRight: string;
    itemPaddingBottom: string;
    itemPaddingLeft: string;
    itemMarginTop: string;
    itemMarginRight: string;
    itemMarginBottom: string;
    itemMarginLeft: string;
    itemFontFamily: string;
    itemFontSize: string;
    itemFontStyle: string;
    itemFontWeightBold: string;
    itemFontLetterSpacing: string;
    itemFontLineHeight: string;
    itemContentLeftColor: string;
    itemContentRightColor: string;
    itemContentLeftWidth: string;
    itemContentRightWidth: string;
    footerBackground: string;
    footerWidth: string;
    footerHeight: string;
    footerPaddingTop: string;
    footerPaddingRight: string;
    footerPaddingBottom: string;
    footerPaddingLeft: string;
    footerMarginTop: string;
    footerMarginRight: string;
    footerMarginBottom: string;
    footerMarginLeft: string;
    footerFontFamily: string;
    footerFontSize: string;
    footerFontStyle: string;
    footerFontWeightBold: string;
    footerFontLetterSpacing: string;
    footerFontLineHeight: string;
    headerBackground: string;
    headerWidth: string;
    headerHeight: string;
    headerPaddingTop: string;
    headerPaddingRight: string;
    headerPaddingBottom: string;
    headerPaddingLeft: string;
    headerMarginTop: string;
    headerMarginRight: string;
    headerMarginBottom: string;
    headerMarginLeft: string;
    headerFontFamily: string;
    headerFontSize: string;
    headerFontStyle: string;
    headerFontWeightBold: string;
    headerFontLetterSpacing: string;
    headerFontLineHeight: string;
    dividerColor: string;
    dividerWidth: string;
    dividerHeight: string;
    dividerMarginTop: string;
    dividerMarginRight: string;
    dividerMarginBottom: string;
    dividerMarginLeft: string;
    groupBackground: string;
    groupWidth: string;
    groupHeight: string;
    groupPaddingTop: string;
    groupPaddingRight: string;
    groupPaddingBottom: string;
    groupPaddingLeft: string;
    groupMarginTop: string;
    groupMarginRight: string;
    groupMarginBottom: string;
    groupMarginLeft: string;
    groupLabelColor: string;
    groupLabelFontFamily: string;
    groupLabelFontSize: string;
    groupLabelFontStyle: string;
    groupLabelFontWeight: string;
    groupLabelLetterSpacing: string;
    groupLabelLineHeight: string;
    groupLabelPaddingTop: string;
    groupLabelPaddingRight: string;
    groupLabelPaddingBottom: string;
    groupLabelPaddingLeft: string;
    groupLabelMarginTop: string;
    groupLabelMarginRight: string;
    groupLabelMarginBottom: string;
    groupLabelMarginLeft: string;
    targetColor: string;
    targetLabelColor: string;
    targetArrowColor: string;
    targetArrowRight: string;
    targetBackgroundColor: string;
    targetBackgroundColorHover: string;
    targetBackgroundColorActive: string;
    targetBackgroundColorOpen: string;
    targetBorderColor: string;
    targetBorderColorHover: string;
    targetBorderColorActive: string;
    targetBorderColorOpen: string;
    targetInnerTop: string;
    targetPadding: string;
    targetPaddingWithInput: string;
    targetPaddingHasChips: string;
    targetHeight: string;
    targetWidth: string;
    targetRadius: string;
    targetFontFamily: string;
    targetFontSize: string;
    targetFontStyle: string;
    targetFontWeight: string;
    targetLetterSpacing: string;
    targetLineHeight: string;
    targetLabelInnerTop: string;
    targetLabelInnerFontFamily: string;
    targetLabelInnerFontSize: string;
    targetLabelInnerFontStyle: string;
    targetLabelInnerFontWeight: string;
    targetLabelInnerLetterSpacing: string;
    targetLabelInnerLineHeight: string;
    chipGap: string;
    chipBackground: string;
    chipColor: string;
    chipBackgroundHover: string;
    chipColorHover: string;
    chipScaleHover: string;
    chipBackgroundReadOnly: string;
    chipColorReadOnly: string;
    chipBackgroundReadOnlyHover: string;
    chipColorReadOnlyHover: string;
    chipBackgroundActive: string;
    chipColorActive: string;
    chipScaleActive: string;
    chipFocusColor: string;
    chipCloseIconColor: string;
    chipOutlineSize: string;
    chipBorderRadius: string;
    chipWidth: string;
    chipHeight: string;
    chipPaddingTop: string;
    chipPaddingRight: string;
    chipPaddingBottom: string;
    chipPaddingLeft: string;
    chipCloseIconSize: string;
    chipFontFamily: string;
    chipFontSize: string;
    chipFontStyle: string;
    chipFontWeight: string;
    chipLetterSpacing: string;
    chipLineHeight: string;
    chipClearContentMarginLeft: string;
    chipClearContentMarginRight: string;
    chipOpacityReadonly: string;
    disabledOpacity: string;
    focusColor: string;
};
//# sourceMappingURL=Combobox.tokens.d.ts.map