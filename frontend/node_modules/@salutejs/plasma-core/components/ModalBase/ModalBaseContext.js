"use strict";

Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.getIdLastModal = exports.getIdFirstModal = void 0;
/**
 * Взаимодействие с модальными оконами.
 */
var getModals = function getModals(items) {
  return items.filter(function (item) {
    var _item$info;
    return item === null || item === void 0 || (_item$info = item.info) === null || _item$info === void 0 ? void 0 : _item$info.isModal;
  });
};
var getIdLastModal = exports.getIdLastModal = function getIdLastModal(items) {
  var modals = getModals(items);
  if (!modals.length) {
    return;
  }
  return modals[modals.length - 1].id;
};
var getIdFirstModal = exports.getIdFirstModal = function getIdFirstModal(items) {
  var modals = getModals(items);
  if (!modals.length) {
    return;
  }
  return modals[0].id;
};